...
### 实现色板（代码，`shapes.PIECE_PALETTE`）

与上表粉彩草案**尚未逐 hex 对齐**。当前生产值为：
...
舞台/托盘：`--stage` 天空蓝渐变；棋盘 `#eef2f5` + 圆角浅格。  
体积数字：**正式局仍显示**（白字）；横/竖仅 `?debug`。  
换皮提交：`0f8c3aa`（`style.css` · `view.ts` · `shapes.ts`）。
