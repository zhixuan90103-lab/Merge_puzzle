...
| 文档 | 用途 |
|------|------|
| **[docs/GAME_RULES.md](./docs/GAME_RULES.md)** | **玩法规则真源 v0.6** |
| **[docs/ARCHITECTURE_GAME.md](./docs/ARCHITECTURE_GAME.md)** | 玩法模块与数据流 |
| **[docs/CHANGELOG_PROTOTYPE.md](./docs/CHANGELOG_PROTOTYPE.md)** | 变更汇总 v0.6 |
...
2. **合**：同色同数 + 朝向；重叠≥1；**合后形由摆放决定**  
3. **推**：障碍 **&lt; 合后 2V**；同体积不可推  
4. **流放**：盘上无某色 → 本波不出该色  
5. **64** → 清盘按关卡表重摆  
6. 失败 = 不能搬且不能合  
7. 结算：`isPlayable` 快路径；禁每帧 full `tryMerge`  
